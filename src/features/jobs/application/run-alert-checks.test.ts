...
describe("runAlertChecks", () => {
  ...
  it("separates a profile's digests when symbols have different latest completed dates", async () => {
    const deps = createDependencies();
    const petrSignal = createSignal(
      "signal-1",
      "profile-1",
      "PETR4",
      "2026-07-13",
    );
    const valeSignal = createSignal(
      "signal-2",
      "profile-1",
      "VALE3",
      "2026-07-12",
    ...
    vi.mocked(deps.marketDataProvider.fetchDailyPrices).mockImplementation(
      async (symbol) =>
        createSnapshots(
          symbol === "PETR4" ? "2026-07-13" : "2026-07-12",
          symbol,
        ),
    );
    vi.mocked(deps.signalRepository.upsertMany)
  ...
  it("requests six months of prices when bootstrapping a symbol", async () => {
    ...
    expect(deps.marketDataProvider.fetchDailyPrices).toHaveBeenNthCalledWith(
      1,
      "PETR4",
      { endDate: "2026-07-14", startDate: "2026-01-14" },
    );
    expect(deps.marketDataProvider.fetchDailyPrices).toHaveBeenCalledTimes(1);
  });
  ...
  it("merges the fetched window with stored prices before calculating indicators", async () => {
    ...
    expect(deps.marketDataProvider.fetchDailyPrices).toHaveBeenCalledWith(
      "PETR4",
      { endDate: "2026-07-14", startDate: "2026-07-12" },
    );
    expect(
  ...
  it("refreshes through the current date when the completed date is already cached", async () => {
    const deps = createDependencies();
    vi.mocked(
      deps.alertCheckTargetRepository.listEnabledTargets,
    ).mockResolvedValue([createTarget("profile-1", "PETR4")]);
    ...
    vi.mocked(deps.marketDataProvider.fetchDailyPrices).mockResolvedValue(
      createSnapshots("2026-07-14", "PETR4", 2),
    );
    ...
    expect(deps.marketDataProvider.fetchDailyPrices).toHaveBeenCalledWith(
      "PETR4",
      { endDate: "2026-07-14", startDate: "2026-07-13" },
    );
    expect(
  ...
  it("keeps retry alert calculations bounded while preserving fresh prices", async () => {
    const deps = createDependencies();
    vi.mocked(
      deps.alertCheckTargetRepository.listEnabledTargets,
    ).mockResolvedValue([createTarget("profile-1", "PETR4")]);
    ...
    expect(
      vi
        .mocked(deps.indicatorSnapshotRepository.upsertMany)
        .mock.calls[0]?.[0].at(-1),
    ).toEqual(expect.objectContaining({ marketDate: "2026-07-14" }));
    expect(deps.signalRepository.upsertMany).toHaveBeenCalledWith(
      expect.not.arrayContaining([
        expect.objectContaining({ marketDate: "2026-07-14" }),
      ]),
    );
    expect(
      deps.alertCheckCheckpointRepository.markProcessed,
  ...
  it("emails a newly available signal from the latest completed date", async () => {
    const deps = createDependencies();
  ...
  it("stores current-day prices without using them for signals or email", async () => {
    const deps = createDependencies();
    vi.mocked(
      deps.alertCheckTargetRepository.listEnabledTargets,
    ).mockResolvedValue([createTarget("profile-1", "PETR4")]);
    vi.mocked(deps.marketDataProvider.fetchDailyPrices).mockResolvedValue(
      createSnapshots("2026-07-14", "PETR4", 44),
    );
    vi.mocked(deps.signalRepository.upsertMany).mockResolvedValue([]);
    ...
    expect(deps.marketDataProvider.fetchDailyPrices).toHaveBeenCalledWith(
      "PETR4",
      expect.objectContaining({ endDate: "2026-07-14" }),
    );
